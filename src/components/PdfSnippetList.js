...
const PdfSnippetList = ({ documents }) => {
  ...
  const grouped = groupByFile(documents);
      ...
      { Array.isArray(documents) && documents.length > 0 && (
        <>
          ...
          <div>
                  ...
                  <ul className="my-3 space-y-2 pl-4 border-l-2 border-gray-300">
                    {snippets.map((item, i) => (
                      <li key={i} className='flex justify-between'>
                        <div
                          className="font-montserrat font-normal text-[12px]"
                        >
                        ...
                        <div className='text-[10px] lg:text-[12px] text-[#8F8989]'>pagina {item.page}</div>
                      </li>
                    ))}
                  </ul>
                )}
            ...
            <div className="h-[2px] w-full bg-[#C5BEBE]"></div>
          </div>
        </>
      )}
    </div>
