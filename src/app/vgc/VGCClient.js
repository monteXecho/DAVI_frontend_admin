...
const STORAGE_KEY_HISTORY_VGC = 'uploadHistory_VGCClient';

const getStoredHistory = () => {
  const storedHistory = JSON.parse(localStorage.getItem(STORAGE_KEY_HISTORY_VGC)) || [];
  return storedHistory;
};

const saveHistoryToLocalStorage = (history) => {
  localStorage.setItem(STORAGE_KEY_HISTORY_VGC, JSON.stringify(history));
};

export default function VGCClient() {
      ...
      <div className="w-full h-full flex flex-col px-[27px] py-9 lg:py-[143px] lg:px-[97px] overflow-scroll scrollbar-hide">
        <section className="flex flex-col gap-[31px]">
          ...
          <p className="text-black font-montserrat font-normal text-lg leading-6">
            Upload je personeels- of roosterdocument en controleer of het vaste gezichten criterium juist is ingevoerd.
          </p>
        </section>
        ...
        <section className="flex flex-col gap-[52px]">
          ...
          {!uploadStatus.UPLOADING 
            ?  <UploadBttn onClick={handleUploadClick} text='Upload rooster' />
            : <></>
          }
        </section>
      </div>
