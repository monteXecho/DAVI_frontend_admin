...
import DocumentUsageHistoryModal from './DocumentUsageHistoryModal';

/** API may return counts as numbers or strings; coerce for display and sorting. */
...
const PAGE_SIZE = 10;
/** Pagination control (numbered pages + ellipsis) — matches dashboard teal */
...
export default function DocumentsInUseClient() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (roleFilter && roleFilter !== ROLE_ALL_LABEL) params.role = roleFilter;
      if (folderFilter && folderFilter !== FOLDER_ALL_LABEL) params.folder = folderFilter;
      const q = debouncedSearch.trim();
      if (q) params.q = q;

      const data = await getCompanyDashboardDocumentsInUse(params);
      ...
      if (Array.isArray(data?.folder_options) && data.folder_options.length) {
        setFolderOptions(data.folder_options);
      }
    } catch (e) {
      console.error(e);
  ...
  const tableRows = useMemo(() => {
    return rows.map((r) => ({
      ...r,
      answer_usage_count: safeUsageCount(r.answer_usage_count),
      last_answer_sort: r.last_answer_at
        ? new Date(r.last_answer_at).getTime()
        : 0,
    }));
  }, [rows]);
  ...
  const headLine = useMemo(
    () => `${total} documenten in gebruik`,
    [total]
  );

  const openUsageHistory = async (row) => {
    if (!row?.id) return;
    setHistoryRow(row);
    setHistoryLoading(true);
    setHistoryEvents([]);
    setHistoryFileName(row.file_name || '');
    try {
      const data = await getCompanyDashboardDocumentAnswerUsageHistory(row.id);
      setHistoryEvents(Array.isArray(data?.events) ? data.events : []);
      setHistoryFileName(data?.file_name || row.file_name || '');
    } catch (e) {
      console.error(e);
      toast.error('Kon gebruiksgeschiedenis niet laden.');
      setHistoryRow(null);
    } finally {
  ...
  const openFileFromHistoryModal = async () => {
    if (!historyRow?.path) {
      toast.warn('Geen bestandspad beschikbaar.');
      return;
    }
    try {
      await openDocumentInNewTab(historyRow.path);
    } catch (e) {
      console.error(e);
      toast.error(e?.message || 'Openen in nieuw tabblad mislukt.');
    }
  };
  ...
  return (
    <div className="w-full h-fit min-h-full flex flex-col py-[81px] overflow-y-auto scrollbar-hide">
      ...
      <div className="w-full px-[102px] py-[46px]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 items-center min-h-[60px] bg-[#F9FBFA] px-2 mb-0 py-2">
        ...
        <div className="overflow-x-auto border border-t-0 border-[#E5E5E5] rounded-b-lg relative">
          {loading && (
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-[#F9FBFA]">
              <tr className="h-[51px] border-b border-[#C5BEBE]">
                ...
                <th className="px-2 w-[130px]" aria-label="Acties" />
              </tr>
            </thead>
            <tbody>
              {sorted.map((row) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-montserrat text-sm text-[#342222]">
                    {formatNlDateTime(row.last_answer_at)}
                  </td>
                  <td className="px-4 py-3 text-sm text-[#342222]">
                    {safeUsageCount(row.answer_usage_count)}
                  </td>
                  <td className="px-2 py-3">
                    <div className="flex justify-end items-center gap-3">
                      <button
                        type="button"
                        onClick={() => openUsageHistory(row)}
                        disabled={historyLoading && historyRow?.id === row.id}
                        className="p-0 border-0 bg-transparent cursor-pointer hover:opacity-85 disabled:opacity-50 outline-none focus:outline-none focus-visible:outline-none focus:ring-0"
                        title="Gebruiksgeschiedenis voor antwoorden"
                        aria-label="Gebruiksgeschiedenis voor antwoorden"
                      >
                        <OpenDocumentCircleIcon />
                      </button>
                      <button
                        type="button"
      ...
      <DocumentUsageHistoryModal
        open={Boolean(historyRow)}
        onClose={closeHistory}
        fileName={historyFileName}
        events={historyEvents}
        loading={historyLoading}
        filePath={historyRow?.path}
        onOpenFile={openFileFromHistoryModal}
      />
    </div>
  );
}
