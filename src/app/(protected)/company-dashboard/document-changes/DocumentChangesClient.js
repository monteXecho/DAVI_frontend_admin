...
import OpenDocumentCircleIcon from '../documents-in-use/OpenDocumentCircleIcon';
...
const USER_DEFAULT = 'Filter op gebruiker';
const ACTIVITY_DEFAULT = 'Filter op activiteit';
const PERIOD_DEFAULT = 'Filter op periode';
...
const ACTIVITY_OPTIONS = [
  { label: 'Geupload', value: 'upload' },
  { label: 'Aangepast', value: 'update' },
  { label: 'Verwijderd', value: 'delete_private' },
];
...
/** Same blue circular document icon as /documents-in-use (next to green edit + red delete). */
function RowDocumentIcon() {
  return (
    <span className="inline-flex items-center justify-center shrink-0" aria-hidden>
      <OpenDocumentCircleIcon className="w-[21px] h-[21px]" />
    </span>
  );
}

export default function DocumentChangesClient() {
  const router = useRouter();
  const { getCompanyDashboardDocumentChanges } = useApi();
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await getCompanyDashboardDocumentChanges(params);
      ...
      console.error(e);
      toast.error('Kon documentenactiviteit niet laden.');
      setRows([]);
      setTotal(0);
    } finally {
      setLoading(false);
  ...
  const headLine = 'Wijzigingen in documenten';
        ...
        <div className="overflow-x-auto border border-t-0 border-[#E5E5E5] rounded-b-lg relative mt-4">
          ...
          <table className="w-full text-left border-collapse min-w-[880px]">
            <thead className="bg-[#F9FBFA]">
              <tr className="h-[51px] border-b border-[#C5BEBE]">
                <th className="w-10 px-2 align-middle">
                  <input type="checkbox" disabled className="rounded border-[#C5BEBE]" aria-hidden />
                </th>
                <SortableHeader
                  sortKey="file_name"
                  onSort={requestSort}
            ...
            <tbody>
              {sorted.map((row, idx) => (
                <tr
                  key={`${row.when}-${row.who_id}-${row.file_name}-${idx}`}
                  className="border-b border-[#C5BEBE] hover:bg-[#F9FBFA] transition"
                >
                  <td className="px-2 py-3 align-middle">
                    <input type="checkbox" disabled className="rounded border-[#C5BEBE]" aria-hidden />
                  </td>
                  <td className="px-4 py-3 font-montserrat text-sm text-[#342222] max-w-[220px] break-words">
                    {row.file_name || '—'}
                  </td>
          ...
          {rows.length === 0 && !loading && (
            <div className="p-8 text-center text-gray-500 font-montserrat">
              Geen documentactiviteit in dit overzicht.
            </div>
          )}
        </div>
