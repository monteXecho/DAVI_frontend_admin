...
export default function DocumentChatUnansweredClient() {
  const router = useRouter();
  const { getCompanyDashboardDocumentChatUnanswered } = useApi();
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const listData = await getCompanyDashboardDocumentChatUnanswered({
        page,
        page_size: PAGE_SIZE,
  ...
  const headLine = useMemo(() => {
    const n = total.toLocaleString('nl-NL');
    return `${n} vragen gesteld zonder antwoord`;
  }, [total]);
        ...
        <div className="mt-4 border border-[#E5E5E5] rounded-lg relative overflow-hidden">
          ...
          <ul className="divide-y divide-[#C5BEBE]">
            {rows.map((row) => (
              <li
                key={row.id}
                className="px-4 py-4 hover:bg-[#F9FBFA] transition"
              >
                <p className="font-montserrat text-sm text-[#342222] leading-snug min-w-0">
                  {row.question_text || '—'}
                </p>
              </li>
            ))}
