...
export default function DeleteRoleModal({ roleName, users, onConfirm, onClose, isMultiple }) {
    ...
    <div className="relative w-[350px] h-[350px] bg-white shadow-md rounded-2xl flex flex-col items-center justify-center gap-10">
      ...
      {isMultiple ? (
        <div className="text-center text-[18px] leading-6 text-black px-6">
          <p className="mb-4">
            Weet je zeker dat je de rol<br />
            <span className="font-semibold">&quot;{roleName}&quot;</span>
            <br />
            wilt verwijderen?
          </p>
          <div className="text-sm text-gray-600 mb-2">
            Van {userCount} gebruiker{userCount !== 1 ? 's' : ''}:
          </div>
      ...
      >
        {isMultiple ? `Verwijder rol van ${userCount} gebruikers` : 'Verwijder rol'}
      </button>
    </div>
  );
}
