...
export default function DocumentenTab({ 
  ...
  onUploadTab, 
  onDeleteDocuments,
  userDocumentFilters = { roles: [], folders: [], userName: "" }
}) {
  ...
  useEffect(() => {
    ...
    let availableRoles = ["Alle Rollen", ...roleNames]
    if (userDocumentFilters.roles.length > 0) {
      availableRoles = ["Alle Rollen", ...userDocumentFilters.roles]
    }
    setAllOptions1(availableRoles)
  }, [roles, userDocumentFilters.roles])
  ...
  useEffect(() => {
    if (selectedRole === "Alle Rollen") {
      ...
      let availableFolders = ["Alle Mappen", ...roleFolders]
      if (userDocumentFilters.folders.length > 0) {
        availableFolders = availableFolders.filter(folder => 
          folder === "Alle Mappen" || userDocumentFilters.folders.includes(folder)
        )
      }
      setAllOptions2(availableFolders)
    } else {
      setAllOptions2(["Alle Mappen"])
    }
  }, [selectedRole, documents, userDocumentFilters.folders])
  ...
  const getAllDocuments = useCallback(() => {
    ...
    Object.entries(documents).forEach(([roleName, roleData]) => {
      if (userDocumentFilters.roles.length > 0 && !userDocumentFilters.roles.includes(roleName)) return
      roleData.folders.forEach(folder => {
        if (userDocumentFilters.folders.length > 0 && !userDocumentFilters.folders.includes(folder.name)) return
  ...
  }, [documents, userDocumentFilters.folders])
  ...
  const getDocumentsForRoleAndFolder = useCallback((role, folder) => {
    ...
    return documents[role].folders.flatMap(folderData => {
      if (folder === "Alle Mappen" || folderData.name === folder) {
        if (userDocumentFilters.folders.length > 0 && !userDocumentFilters.folders.includes(folderData.name)) return []
  ...
  const getHeaderText = () => {
    if (userDocumentFilters.userName) {
      return `Documenten voor ${userDocumentFilters.userName} (${filteredDocuments.length})`
    }
  ...
  const selectedUser = users.find(u => u.name === userDocumentFilters.userName) || null
    ...
    <div className="flex flex-col w-full">
      ...
      {selectedUser && (
        <div className="mb-[20px] font-montserrat font-medium text-[16px] text-gray-700">
          ...
          {selectedUser.roles && selectedUser.roles.length > 0 && (
            <span> — Rollen: {selectedUser.roles.join(", ")}</span>
          )}
        </div>
      )}
      ...
      {filteredDocuments.length === 0 ? (
        <div className="text-center py-4 text-gray-500 font-montserrat">
          {userDocumentFilters.userName
            ? `Geen documenten beschikbaar voor ${userDocumentFilters.userName}`
          ...
          <table className="w-full border-collapse text-left">
            <thead className="bg-[#F9FBFA]">
              <tr className="h-[51px] border-b border-[#C5BEBE]">
                ...
                <th className="w-[100px] px-4 py-2 font-montserrat font-bold text-[16px] text-black text-center">
                  Acties
                </th>
              </tr>
            </thead>
