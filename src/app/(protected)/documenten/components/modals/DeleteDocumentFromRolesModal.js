// components/modal/DeleteDocumentFromRolesModal.jsx
import { X } from "lucide-react"

export default function DeleteDocumentFromRolesModal({ roles, documentName, onConfirm, onClose, isMultiple }) {
  return (
    <div className="relative w-[350px] h-[350px] bg-white shadow-md rounded-2xl flex flex-col items-center justify-center gap-10">
      ...
      {isMultiple ? (
        <div className="text-center text-[18px] leading-6 text-black px-6">
          <p className="mb-4">
            ...
            <br />
            wil verwijderen uit <span className="font-semibold">{roles.length} rollen</span>?
          </p>
      ...
      >
        {isMultiple ? `Verwijder uit ${roles.length} rollen` : 'Verwijder uit rol'}
      </button>
    </div>
  )
}