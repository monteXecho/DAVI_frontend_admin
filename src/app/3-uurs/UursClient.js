...
const STORAGE_KEY_HISTORY_UURS = 'uploadHistory_UURSClient';

const getStoredHistory = () => {
  const storedHistory = JSON.parse(localStorage.getItem(STORAGE_KEY_HISTORY_UURS)) || [];
  return storedHistory;
};

const saveHistoryToLocalStorage = (history) => {
  localStorage.setItem(STORAGE_KEY_HISTORY_UURS, JSON.stringify(history));
};

export default function UursClient() {
      ...
      <div className="w-full h-full flex flex-col px-[27px] py-9 lg:py-[143px] lg:px-[97px] overflow-scroll scrollbar-hide">
        <section className="flex flex-col gap-[31px]">
          ...
          <p className="text-black font-montserrat font-normal text-lg leading-6">
            Upload je planning om direct te controleren of de 3-uursregeling correct is toegepast.
          </p>
        </section>
        ...
        <section className="flex flex-col gap-[52px]">
          ...
          {!uploadStatus.UPLOADING 
            ?  <UploadBttn onClick={handleUploadClick} text='Upload planning' />
            : <></>
          }
        </section>
      </div>
